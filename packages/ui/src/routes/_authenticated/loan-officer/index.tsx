...
function PipelineMetrics({ applications }: { applications: ApplicationResponse[] }) {
  ...
  const inUnderwriting = applications.filter(
    (a) => a.stage === 'underwriting' || a.stage === 'conditional_approval',
  );
  const critical = applications.filter((a) => a.urgency?.level === 'critical');
  const avgDays =
    active.length > 0
      ? Math.round(
          active.reduce((sum, a) => sum + (a.urgency?.days_in_stage ?? 0), 0) /
            active.length,
        )
      : 0;
...
const FILTER_STAGES: { value: ApplicationStage | ''; label: string }[] = [
  ...
  { value: 'prequalification', label: '预审' },
  { value: 'application', label: '申请中' },
  { value: 'processing', label: '材料处理中' },
  { value: 'underwriting', label: '授信审批' },
  { value: 'conditional_approval', label: '审批决策' },
  { value: 'clear_to_close', label: '具备放款条件' },
...
function LoanOfficerPipeline() {
  ...
  return (
    <div className="mx-auto max-w-[1280px] p-6 md:p-8">
      <div className="mb-6">
        <h1 className="text-2xl font-bold text-foreground">客户经理业务看板</h1>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-border bg-slate-50 dark:bg-slate-800/50">
                ...
                >
                  当前阶段
                  <SortIcon sortKey="stage" activeKey={sortKey} dir={sortDir} />
                </th>
                <th
                ...
                >
                  阶段时长
                  <SortIcon sortKey="days_in_stage" activeKey={sortKey} dir={sortDir} />
                </th>
                ...
                >
                  最近更新
                  <SortIcon sortKey="last_activity" activeKey={sortKey} dir={sortDir} />
                </th>
            ...
            <tbody>
              {isLoading ? (
                ...
                filtered.map((app) => <PipelineRow key={app.id} app={app} />)
              )}
            </tbody>
          </table>
        </div>
  ...
  return (
    ...
    >
      <td className="px-4 py-4">
        <span
          className={cn(
            ...
            URGENCY_DOT[urgencyLevel] ?? URGENCY_DOT.normal,
          )}
      ...
      <td className="px-4 py-4 text-foreground">
        {formatDays(app.urgency?.days_in_stage)}
      </td>
