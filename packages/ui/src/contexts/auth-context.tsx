      ...
      if (stored.refresh_token) {
        ...
        const doRefresh = (rt: string) => {
          fetch(tokenUrl, {
            ...
            .then((r) =>
              r.ok ? r.json() : Promise.reject(new Error('Refresh failed')),
            )
            .then((d) => {
              ...
              const roles =
                ((newClaims.realm_access as Record<string, unknown>)
                  ?.roles as string[]) ?? [];
              const roleMatch = roles.find((r2) => KNOWN_ROLES.has(r2));
              const role: UserRole =
                roleMatch === 'admin' ? 'ceo' : ((roleMatch as UserRole) ?? 'borrower');
              const newUser: AuthUser = {
                role,
                user_id: (newClaims.sub as string) ?? '',
                name:
                  (newClaims.name as string) ??
                  (newClaims.preferred_username as string) ??
                  '',
              ...
              const nextMs = newExp
                ? Math.max(newExp * 1000 - Date.now() - 60_000, 10_000)
                : 840_000;
            ...
            fetch(tokenUrl, {
              ...
              .then((d) => {
                ...
                const newRole: UserRole =
                  newRoleMatch === 'admin'
                    ? 'ceo'
                    : ((newRoleMatch as UserRole) ?? 'borrower');
                const newUser: AuthUser = {
                  role: newRole,
                  user_id: (newClaims.sub as string) ?? '',
                  name:
